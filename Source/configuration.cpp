...
void Configuration::ChargerConf()
{
    ...
    if(fichier)
    {
        ...
        {
            if(chaine== "resolution:") { fichier>>Resolution.x>>Resolution.y; Resolution.w=Resolution.x; Resolution.h=Resolution.y; }
...
void Configuration::ChargerInit()
{
    ...
    if(fichier)
    {
        ...
        while(fichier>>chaine)
        {
            ...
            if(chaine== "items:") { fichier>>chemin_items; }
            if(chaine== "temps:") { fichier>>chemin_temps; }
            if(chaine== "saves:") { fichier>>chemin_saves; }
            if(chaine== "events:") { fichier>>chemin_evenements; }
            if(chaine== "cursor:") { fichier>>chemin_curseurs; }
            if(chaine== "menus:") { fichier>>chemin_menus; }
            if(chaine== "fonts:") { fichier>>chemin_fonts; }
            if(chaine== "fx:") { fichier>>chemin_fx; }
            if(chaine== "alpha_arts:") { fichier>>chemin_aa; }

            if(chaine== "basic_cursor:") { fichier>>nom_curseur_base; }
            if(chaine== "fxBlack:") { fichier>>nom_effetNoir; }
            if(chaine== "fxDeath:") { fichier>>nom_effetMort; }
            if(chaine== "fxContrast:") { fichier>>nom_effetContrastes; }
            if(chaine== "fxBlur:") { fichier>>nom_effetBlur; }

            if(chaine== "menu_minimap:") { fichier>>nom_minimap; }

            if(chaine== "menu_soul:") { fichier>>nom_ame; }
            if(chaine== "menu_soul_orb:") { fichier>>nom_barre_ame; }
            if(chaine== "menu_life_monster:") { fichier>>nom_barre_vie; }
            if(chaine== "menu_life_monster_blank:") { fichier>>nom_barre_vie_vide; }
            if(chaine== "menu_bag:") { fichier>>nom_sac; }

            if(chaine== "text_benedictions:") { fichier>>chemin_text_benedictions; }
            if(chaine== "text_menus:") { fichier>>chemin_text_menus; }

            if(chaine== "font_title:") { fichier>>font_titre; }
            if(chaine== "menu_blood:") { fichier>>nom_sang; }
            if(chaine== "cache: ") { fichier>>cache; }
            if(chaine== "death_sound:") { fichier>>chemin_son_mort; }
