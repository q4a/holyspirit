...
Map::Map()
{
    ...
    carreBrun.Create(8*configuration.Resolution.x/800, 8*configuration.Resolution.y/600, Color(128, 64, 0)),carreBleu.Create(8*configuration.Resolution.x/800, 8*configuration.Resolution.y/600, Color(32, 0, 128)),carreRouge.Create(8*configuration.Resolution.x/800, 8*configuration.Resolution.y/600, Color(128, 0, 0));
}
    ...
    if(fichier)
    {
    	...
    	do
    	{
    		...
    		if(caractere=='*')
    		{
                ...
                if(!m_musique.Open(cheminDeLaMusique.c_str()))
    	...
    	do
    	{
    		...
    		if(caractere=='*')
    		{
    			fichier>>m_lumiere.rouge;
    			fichier>>m_lumiere.vert;
    			fichier>>m_lumiere.bleu;
    			fichier>>m_lumiere.intensite;
    		}
    	}while(caractere!='$');
    	Tileset tilesetTemp;
    	do
                ...
                if(caractere=='*')
                {
                   // if(couche==0)
                    ...
                    do
                    {
                        ...
                        tileFinal = (rand() % (tile.size() -1 - 0 + 1)) + 0;
                        ...
                        tileset=-1,tile.clear(),evenement=-1,monstre=-1,herbe=-1;
                        position.x++;
                        fichier.get(caractere);
                    }while(caractere!='\n');
                    position.x=0;
...
void Map::calculerOmbresEtLumieres(sf::RenderWindow* ecran,Hero *hero)
{
    ...
    lampesMin.x=hero->m_personnage.getCoordonnee().x-15;
    lampesMin.y=hero->m_personnage.getCoordonnee().y-15;
    lampesMax.x=hero->m_personnage.getCoordonnee().x+15;
    lampesMax.y=hero->m_personnage.getCoordonnee().y+15;
    ...
    m_lumiereHero=m_lumiere;
    if(configuration.Ombre)
        m_lumiereHero.AjouterOmbre(m_lumiere.intensite/4,45);


        // Je fonctionne comme pour en haut, juste que je prend le héro comme source de lumière, pour la portée lumineuse
    for(int i=0;i<vueMax.y-vueMin.y;i++)
        for(int j=0;j<vueMax.x-vueMin.x;j++)
        {
             m_tableauDesLampes[i][j]=m_lumiere;
            if(configuration.Ombre)
                m_tableauDesLampes[i][j].AjouterOmbre(m_lumiere.intensite/4,45);

            lumiereTemp=0;
            lumiereTemp=(float)hero->m_modelePersonnage.getPorteeLumineuse().intensite-((((float)sqrt((hero->m_personnage.getCoordonneePixel().x-(j+vueMin.x)*COTE_TILE)*(hero->m_personnage.getCoordonneePixel().x-(j+vueMin.x)*COTE_TILE)+(hero->m_personnage.getCoordonneePixel().y-(i+vueMin.y)*COTE_TILE)*(hero->m_personnage.getCoordonneePixel().y-(i+vueMin.y)*COTE_TILE))))/COTE_TILE)*30;
            ...
            if(configuration.Ombre)
            {
                ...
                m_tableauDesLampes[i][j].AjouterOmbre(lumiere.intensite/2,(int)angle);
            }
        }
        ...
        for(int couche=1;couche==1;couche++)
        {
            for(int j=lampesMin.y;j<lampesMax.y;j++)
            {
                ...
                {
                    bool ok=false;
                    if(m_decor[couche][j][k].getMonstre()>=0&&m_decor[couche][j][k].getMonstre()<m_monstre.size())// Je vérifie que le tile est initialisé ^^
                    ok=true;
                    ...
                    if(ok)
                    {
                        ...
                        if(m_decor[couche][j][k].getMonstre()>=0&&m_decor[couche][j][k].getMonstre()<m_monstre.size())
                        {
                            lumiereTile.rouge=(lumiereTile.rouge*lumiereTile.intensite+m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().rouge*m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite)/(m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite+lumiereTile.intensite);
                            lumiereTile.vert=(lumiereTile.vert*lumiereTile.intensite+m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().vert*m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite)/(m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite+lumiereTile.intensite);
                            lumiereTile.bleu=(lumiereTile.bleu*lumiereTile.intensite+m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().bleu*m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite)/(m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite+lumiereTile.intensite);

                            lumiereTile.intensite+=m_ModeleMonstre[m_monstre[m_decor[couche][j][k].getMonstre()].getModele()].getPorteeLumineuse().intensite;
                            ...
                            for(int m=vueMin.x;m<vueMax.x;m++)
                                {
                                    ...
                                    lumiereTemp=(float)lumiereTile.intensite-(float)(sqrt((k*COTE_TILE-m*COTE_TILE)*(k*COTE_TILE-m*COTE_TILE)+(j*COTE_TILE-l*COTE_TILE)*(j*COTE_TILE-l*COTE_TILE)))/5;
                                    ...
                                    for(int o=1;o<15;o++)
                                    {
                                        Xtemp=((double)(k-m)/15)*(double)o;
                                        Ytemp=((double)(j-l)/15)*(double)o;
                                    ...
                                    if(lumiere.intensite>0)
                                    {
                                        ...
                                        m_tableauDesLampes[l-vueMin.y][m-vueMin.x].rouge=(lumiere.rouge*lumiere.intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite)+m_tableauDesLampes[l-vueMin.y][m-vueMin.x].rouge*m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite));
                                        m_tableauDesLampes[l-vueMin.y][m-vueMin.x].vert=(lumiere.vert*lumiere.intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite)+m_tableauDesLampes[l-vueMin.y][m-vueMin.x].vert*m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite));
                                        m_tableauDesLampes[l-vueMin.y][m-vueMin.x].bleu=(lumiere.bleu*lumiere.intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite)+m_tableauDesLampes[l-vueMin.y][m-vueMin.x].bleu*m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite/(m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite+lumiere.intensite));
                                        ...
                                        if(configuration.Ombre)
                                        {
                                            ...
                                            m_tableauDesLampes[l-vueMin.y][m-vueMin.x].AjouterOmbre(lumiere.intensite,(int)angle);
                                        }
                                        ...
                                        if(l==hero->m_personnage.getCoordonnee().y&&m==hero->m_personnage.getCoordonnee().x)
                                        {
                                           ...
                                           m_lumiereHero.bleu=(lumiere.bleu*lumiere.intensite/(m_lumiereHero.intensite+lumiere.intensite)+m_lumiereHero.bleu*m_tableauDesLampes[l-vueMin.y][m-vueMin.x].intensite/(m_lumiereHero.intensite+lumiere.intensite));
                                            ...
                                            if(configuration.Ombre)
                                            {
                                                ...
                                                if(lumiereTemp<0)
                                                    lumiereTemp=0;
                                               m_lumiereHero.AjouterOmbre((int)lumiereTemp,(int)angle);
                                            }
            ...
            for(int j=hero->m_personnage.getCoordonnee().y-15;j<hero->m_personnage.getCoordonnee().y+15;j++)
            {
                for(int k=hero->m_personnage.getCoordonnee().x-15;k<hero->m_personnage.getCoordonnee().x+15;k++)
                {
                    ...
                    if(j>=0&&j<m_decor[0].size()&&k>=0&&k<m_decor[0][0].size())
                    {
                        ...
                        {
                            spriteLumiereMask.SetLeft(position.x-128);
                            spriteLumiereMask.SetTop(position.y-64);
                        ...
                        if(m_decor[couche][j][k].getTileset()>=0&&m_decor[couche][j][k].getTileset()<m_tileset.size())
                        {
                            if(position.x+positionPartieDecor.w+positionPartieDecor.h>=ViewRect.Left&&position.x-positionPartieDecor.h-positionPartieDecor.w<ViewRect.Right&&position.y+positionPartieDecor.h+positionPartieDecor.w>=ViewRect.Top&&position.y-positionPartieDecor.h<ViewRect.Bottom+100&&m_tileset[m_decor[couche][j][k].getTileset()].getOmbreDuTile(m_decor[couche][j][k].getTile()&&configuration.Ombre)
                                ...
                                if(configuration.Ombre)
                                {
                                    if(m_tileset[m_decor[couche][j][k].getTileset()].getOmbreDuTile(m_decor[couche][j][k].getTile()))
                                    {
                                        for(int o=0;o<m_tableauDesLampes[j-vueMin.y][k-vueMin.x].m_ombre.size();o++)
                                        {
                                            ...
                                            Sprite.SetLeft(position.x+64-positionPartieDecor.w/2);
                                            Sprite.SetTop(position.y-positionPartieDecor.h*1.5+64);
                                            Sprite.SetScale(1, 1.5f);
                                            Sprite.SetRotationCenter(positionPartieDecor.w/2,positionPartieDecor.h*1.5-32);
                            ...
                            position.y=(k+j)/5;
                                ...
                                if(m_tileset[m_decor[couche][j][k].getTileset()].getSonTile(m_decor[couche][j][k].getTile())>=0)
                                    ...
                                    sqrt((hero->m_personnage.getCoordonnee().x-k)*(hero->m_personnage.getCoordonnee().x-k)+(hero->m_personnage.getCoordonnee().y-j)*(hero->m_personnage.getCoordonnee().y-j)),position,positionHero);
                        }
                    }
                    else
                    {
...
bool Map::getCollision(int positionX,int positionY)
{
	...
	for(int i=0;i<2;i++)
	{
        ...
        if(m_decor[i][positionY][positionX].getMonstre()>-1)
            return 1;
	}
        ...
        for(int j=vueMin.y;j<vueMax.y;j++)
            for(int k=vueMin.x;k<vueMax.x;k++)
            {
                ...
                if(m_decor[i][j][k].getMonstre()>=0&&m_decor[i][j][k].getMonstre()<m_monstre.size())
                    m_monstre[m_decor[i][j][k].getMonstre()].animer(&m_ModeleMonstre[m_monstre[m_decor[i][j][k].getMonstre()].getModele()],m_decor[0].size());
           }
        ...
        if(m_evenement[m_decor[i][hero->m_personnage.getCoordonnee().y][hero->m_personnage.getCoordonnee().x].getEvenement()].getType()==CHANGEMENT_DE_MAP)
        {
            ...
            for(int z=50;z>=0;z-=2)
            {
                ...
                if(configuration.Lumiere)
				{
                    ...
                    calculerOmbresEtLumieres(ecran,hero);
				}
