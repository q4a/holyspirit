...
c_Jeu::c_Jeu(Jeu *)
{
        ...
        if(!jeu->hero.m_modelePersonnage.Charger("Data/Personnages/jeu->hero/GuerrierHache.txt")) // Chargement du héro
        {
            console.Rapport();
            return 0;
        }
...
void c_Jeu::Utiliser(Jeu *)
{
	...
	//{
			...
			if(tempsEcouleDepuisDernierDeplacement>=0.0)
			{
				...
				if(!jeu->map.testEvenement(ecran,&jeu->hero,&camera,&jeu->menu)) // On test les événement pour voir s'il on doit changer de jeu->map, faire des dégats au perso, le régénérer, etc
                {
                    console.Rapport();
                    return 0;
                }
